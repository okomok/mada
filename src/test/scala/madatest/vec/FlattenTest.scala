...
class FlattenTest {
    def testTrivial {
        ...
        val vv = Vector.flatten(Vector.arrayVector(Array(v1, v2, v3, v4, v5)))
    ...
    def testEmpty {
        ...
        val v3 = Vector.arrayVector(detail.Example.empty1)
        val vv = Vector.flatten(Vector.arrayVector(Array(v1, v2, v3)))
        detail.TestEmpty(vv)
    }

    def testFlatMap: Unit = {
        val v = Vector.arrayVector(detail.Example.example1).flatMap(Vector.single(_: Int))
