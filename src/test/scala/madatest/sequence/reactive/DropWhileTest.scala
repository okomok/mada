...
class DropWhileTest {
    def testTrivial0: Unit = {
        ...
        reactive.fromIterative(a).dropWhile(_ <= 4).start(reactor.make(_ => b.add(99), b.add(_)))
    ...
    def testAll: Unit = {
        val a = vector.Of(1,2,3,4,5,6)
        val b = new java.util.ArrayList[Int]
        reactive.fromIterative(a).dropWhile(_ <= 10).start(reactor.make(_ => b.add(99), b.add(_)))
