...
class TakeTest {
    ...
    def testTrivial0: Unit = {
        ...
        reactive.fromIterative(a).take(3).start(reactor.make(_ => b.add(99), b.add(_)))
    ...
    def testTrivial: Unit = {
        ...
        (src(0) merge src(1)).synchronize.take(6).start(dst)
        src.start
    ...
    def testOrder: Unit = {
        ...
        (src(0) merge reactive.empty).synchronize.take(4).start(dst)
        src.start
        src.shutdown(dst.assertMe)
        ()
    }
    ...
    def testNonTrivial: Unit = {
        ...
        (src(0) merge src(1) merge src(2) merge src(3) merge src(4)).synchronize.take(11).start(dst)
        src.start
        src.shutdown(dst.assertMe)
        ()
    }
    ...
    def testFusion: Unit = {
        val src = new IntSenders(ones, ones, ones, ones, ones)
        val dst = new IntReceiver(vector.single(1).times(11))
        (src(0) merge src(1) merge src(2) merge src(3) merge src(4)).synchronize.take(17).take(11).start(dst)
        src.start
        src.shutdown(dst.assertMe)
        ()
    }
    ...
    def testEmpty: Unit = {
        ...
        (src(0) merge src(1)).synchronize.take(3).start(dst)
        src.start
        src.shutdown(dst.assertMe)
        ()
    }
    ...
    def testZero: Unit = {
        val src = new IntSenders(ones, ones, ones, ones, ones)
        val dst = new IntReceiver(vector.empty[Int])
        (src(0) merge src(1) merge src(2) merge src(3) merge src(4)).synchronize.take(0).start(dst)
        src.start
        src.shutdown(dst.assertMe)
        ()
    }
    ...
    def testTooMany: Unit = {
        ...
        (src(0) merge src(1) merge src(2) merge src(3) merge src(4)).synchronize.take(500).start(dst)
        src.start
        src.shutdown(dst.assertMe)
        ()
    }
    ...
    def testBound: Unit = {
        val src = new IntSenders(ones, ones, ones, ones, ones)
        val dst = new IntReceiver(vector.single(1).times(20))
        (src(0) merge src(1) merge src(2) merge src(3) merge src(4)).synchronize.take(20).start(dst)
        src.start
        src.shutdown(dst.assertMe)
        ()
    }
