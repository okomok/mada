...
class UnsplitTest {
    def testTrivial: Unit = {
        ...
        val v = vector.unsplit(vector.fromArray(Array(v1, v2, v3, v4, v5)).toIterable)(vector.of(99))
        assertEquals(vector.of(0,1,2,99,3,4,99,99,5,6,99,7,8,9,10), v)
    }

    def testOne: Unit = {
        val v = vector.unsplit(Vector(vector.from("abc")).toIterable)(vector.of('X'))
        assertEquals(vector.from("abc"), v)
    }

    def testEmpty: Unit = {
        val v = vector.unsplit(vector.fromArray(Array[Vector[Int]]()).toIterable)(vector.of(99))
        detail.TestEmpty(v)
    }
}
