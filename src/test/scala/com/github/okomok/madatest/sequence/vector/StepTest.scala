...
//        println("subvector:" + vector.Region(from(example1), 2, 11).toString)
        val actual = vector.Region(from(example1), 2, 11).step(3)
//        println(actual)
        detail.TeztVectorReadWrite(expected, actual)
    }
    ...
    def testBounds {
        ...
        val actual = from(example1).step(7) // 15 / 7 = 2..1
        detail.TeztVectorReadWrite(expected, actual)
    }

    def testOne {
        val actual = from(example1).step(1)
        detail.TeztVectorReadWrite(example1, actual)
    }

    def testBigStride {
        val v = from(example1).drop(2).step(99)
    ...
    def testStepStep: Unit = {
        ...
        val actual = from(example1).step(3).seal.drop(1).step(2)
        detail.TeztVectorReadWrite(expected, actual)
    }

    def testStepDrop: Unit = {
        val expected = Array(17,23)
        val actual = from(example1).step(3).drop(1).step(2)
        detail.TeztVectorReadWrite(expected, actual)
    }

    def testFusion: Unit = {
        val expected = Array(0,13,15)
        val actual = from(example1).step(3).step(2)
        detail.TeztVectorReadWrite(expected, actual)
    }
}
