...
class FlattenTest extends org.scalatest.junit.JUnit3Suite {
    ...
    def testEmpty {
        ...
        val v2 = iterative.from(detail.Sample.empty1)
        val v3 = vector.from(detail.Sample.empty1)
    ...
    def testFlatMap: Unit = {
        val v = vector.from(detail.Sample.sample1).flatMap(vector.single(_: Int)).toVector
        detail.TeztVectorReadOnly(detail.Sample.sample1, v)
    }
}
