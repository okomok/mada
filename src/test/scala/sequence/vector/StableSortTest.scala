...
class StableSortTest extends org.scalatest.junit.JUnit3Suite {
    def testTrivial {
        val actual = from(sample1).seal.stableSort
    ...
    def testOptimizeArray {
        val actual = from(sample1).stableSort
        detail.TeztVectorReadOnly(sample1Sorted, actual)
    }

    def testOptimizeArrayWindow {
        val actual = from(sample1).window(0, 0).window(0, sample1.length).stableSort
        detail.TeztVectorReadOnly(sample1Sorted, actual)
    }

    def testOptimizeArrayList {
        val actual = from(from(sample1).toJList).stableSort
        detail.TeztVectorReadOnly(sample1Sorted, actual)
    }
...
// vector.stl
    def testStlTrivial: Unit = {
        ...
        assertEquals(from(sample1Sorted), v)
    }

    def testStlBy: Unit = {
        val v = from(sample1)
        mada.sequence.vector.stl.stableSort(v, 0, v.size)(Ordering.Int.reverse)
        assertEquals(from(sample1ReversedSorted), v)
    }
    ...
    def testStlLongSample1: Unit = {
        val v = from(longSample1)
        mada.sequence.vector.stl.stableSort(v, 0, v.size)
