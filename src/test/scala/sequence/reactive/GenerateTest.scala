...
class GenerateTest extends org.scalatest.junit.JUnit3Suite {
    ...
    def testThen: Unit = {
        val s = new java.util.ArrayList[Int]
        for (x <- reactive.origin(mada.eval.ByStrict).generate(Iterative.Of(9,8,7,6,5)).then(s.add(99))) {
            s.add(x)
        }
    ...
    def testThenAppend: Unit = {
        val s = new java.util.ArrayList[Int]
        for (x <- reactive.origin(mada.eval.ByStrict).generate(Iterative.Of(9,8,7,6,5)).then_++(reactive.Of(2,3,4))) {
            s.add(x)
        }
