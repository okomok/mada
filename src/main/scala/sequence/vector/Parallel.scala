...
// associative folding
    ...
    override def folder[B >: A](z: B)(op: (B, B) => B): Vector[B] = ParallelFolder(delegate, z, op, grainSize)
    override def reducer[B >: A](op: (B, B) => B): Vector[B] = ParallelReducer(delegate, op, grainSize)
// conversion
