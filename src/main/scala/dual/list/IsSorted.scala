...
object IsSorted {
    ...
    case class Then[xs <: List, o <: Ordering](xs: xs, o: o) extends Function0 {
        type self = Then[xs, o]
        override  def apply: apply = `if`(o.lt(xs.tail.head, xs.head), const0(`false`), ThenElse(xs, o)).apply.asInstanceOf[apply]
        override type apply        = `if`[o#lt[xs#tail#head, xs#head], const0[`false`], ThenElse[xs, o]]#apply
    }

    case class ThenElse[xs <: List, o <: Ordering](xs: xs, o: o) extends Function0 {
