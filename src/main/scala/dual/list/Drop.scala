...
object Drop {
     def apply[xs <: List, n <: Nat](xs: xs, n: n) =
        `if`(xs.isEmpty.or(n.isZero), const0(xs), Else(xs, n)).apply.asInstanceOfList.asInstanceOf[apply[xs, n]]
...
object DropWhile {
    ...
    case class Else[xs <: List, f <: Function1](xs: xs, f: f) extends Function0 {
        type self = Else[xs, f]
        override  def apply: apply = `if`(f.apply(xs.head).asInstanceOfBoolean, ElseThen(xs, f), const0(xs)).apply.asInstanceOf[apply]
