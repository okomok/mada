...
object Equal {
    ...
    case class Else[xs <: List, ys <: List, ee <: Option](xs: xs, ys: ys, ee: ee) extends Function0 {
        ...
        private lazy val _ee: _ee = ee.getOrElse(function.GetNaturalOrdering(xs.head)).asInstanceOfEquiv.asInstanceOf[_ee]
        private type _ee          = ee#getOrElse[function.GetNaturalOrdering[xs#head]]#asInstanceOfEquiv
