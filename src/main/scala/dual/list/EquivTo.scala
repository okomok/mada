...
object EquivTo {
    ...
    class Else[xs <: List, ys <: List, e <: Equiv](xs: xs, ys: ys, e: e) extends Function0 {
        ...
        override  def apply: apply = `if`(e.equiv(xs.head, ys.head), new ElseThen(xs, ys, e), const0(`false`)).apply.asInstanceOf[apply]
        override type apply        = `if`[e#equiv[xs#head, ys#head],     ElseThen[xs, ys, e], const0[`false`]]#apply
    }

    class ElseThen[xs <: List, ys <: List, e <: Equiv](xs: xs, ys: ys, e: e) extends Function0 {
