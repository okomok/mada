...
trait AbstractMap extends Map {
    ...
    private type _putList[self <: Map, xs <: List]               = PutList.apply[self, xs]
    final override type putList[xs <: List] = _putList[self, xs]
    ...
    private type _equivTo[self <: Map, that <: Map, ve <: Equiv]                                 = EquivTo.apply[self, that, ve]
    final override type equivTo[that <: Map, ve <: Equiv] = _equivTo[self, that, ve]

    final override  def union[that <: Map](that: that): union[that] = that.putList(self.toList)
    private type _union[self <: Map, that <: Map]                   = that#putList[self#toList]
