    ...
    override val key: k, override val value: v, override val left: l, override val right: r) extends BSTree
{
    ...
    override  def toList: toList         = (self.left.toList ::: list.single(Tuple2(self.key, self.value)) ::: self.right.toList).asInstanceOf[toList]
    private type _toList[self <: BSTree] =  self#left#toList ::: list.single[Tuple2[self#key, self#value]] ::: self#right#toList
    override type toList = _toList[self]

    override  def keyList: keyList        = (self.left.keyList ::: list.single(self.key) ::: self.right.keyList).asInstanceOf[keyList]
    private type _keyList[self <: BSTree] =  self#left#keyList ::: list.single[self#key] ::: self#right#keyList
    override type keyList = _keyList[self]

    override  def valueList: valueList      = (self.left.valueList ::: list.single(self.value) ::: self.right.valueList).asInstanceOf[valueList]
    private type _valueList[self <: BSTree] =  self#left#valueList ::: list.single[self#value] ::: self#right#valueList
