...
object NodeRemove {
     def apply[m <: BSTree, k <: Any](m: m, k: k): apply[m, k] =
        m.ord.`match`(k, m.key, CaseLT(m, k), CaseGT(m, k), CaseEQ(m, k)).asMapBSTree.asInstanceOf[apply[m, k]]
    type apply[m <: BSTree, k <: Any] =
        m#ord#`match`[k, m#key, CaseLT[m, k], CaseGT[m, k], CaseEQ[m, k]]#asMapBSTree
