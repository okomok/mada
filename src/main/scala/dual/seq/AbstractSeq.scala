...
trait AbstractSeq extends Seq {
    ...
    private type _partition[self <: Seq, f <: Function1]              = Tuple2[    Filter[self, f],     Filter[self, f#not]]
    ...
    private type _scanLeft[self <: Seq, z <: Any, f <: Function2]                      =     ScanLeft[self, z, f]
    ...
    private type _scanRight[self <: Seq, z <: Any, f <: Function2]                      =      ScanRight[self, z, f]
    ...
    private type _span[self <: Seq, f <: Function1]         = Tuple2[    TakeWhile[self, f],     DropWhile[self, f]]
    ...
    private type _splitAt[self <: Seq, n <: Nat]            = Tuple2[    Take[self, n],     Drop[self, n]]
    ...
    final override  def zip[that <: Seq](that: that): zip[that] = new Zip(self, that)
    private type _zip[self <: Seq,  that <: Seq]                =     Zip[self, that]
    final override type zip[that <: Seq] = _zip[self, that]

    final override  def unzip: unzip = Tuple2(new Unzip1(self), new Unzip2(self))
    private type _unzip[self <: Seq] = Tuple2[    Unzip1[self],     Unzip2[self]]
