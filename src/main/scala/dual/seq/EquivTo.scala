...
final class EquivTo[xs <: Seq, ys <: Seq, e <: Equiv](xs: xs, ys: ys, e: e) extends Function1 {
    type self = EquivTo[xs, ys, e]

    override  def apply: apply =
        `if`(xs.isEmpty  && ys.isEmpty,  Const0(`true`), `if`(xs.isEmpty  !== ys.isEmpty,  Const0(`false`), new Else)).apply.asInstanceOfBoolean.asInstanceOf[apply[xs, ys, e]]
    override type apply =
        `if`[xs#isEmpty# &&[ys#isEmpty], Const0[`true`], `if`[xs#isEmpty# !==[ys#isEmpty], Const0[`false`],     Else]]#apply#asInstanceOfBoolean

    case class Else extends Function0 {
        type self = Else
    ...
    case class ElseThen extends Function0 {
        type self = ElseThen
        override  def apply: apply = new EquivTo(xs.tail, ys.tail, e).apply
