...
private[dual]
final class BSTreeFrom[m <: map.bstree.BSTree](override val impl: m) extends BSTree {
    ...
    override  def add[k <: Any](k: k): add[k]   = new BSTreeFrom(self.impl.put(k, Unit))
    private type _add[self <: BSTree, k <: Any] =     BSTreeFrom[self#impl#put[k, Unit]]
    override type add[k <: Any] = _add[self, k]

    override  def clear: clear          = new BSTreeFrom(self.impl.clear)
    private type _clear[self <: BSTree] =     BSTreeFrom[self#impl#clear]
    override type clear = _clear[self]

    override  def remove[k <: Any](k: k): remove[k] = new BSTreeFrom(self.impl.remove(k))
