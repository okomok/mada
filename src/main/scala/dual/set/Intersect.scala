...
private[mada] final class Intersect {
     def apply[s <: Set, z <: Set](s: s, z: z): apply[s, z] = s.clear.addSeq(s.toList.filter(Pred(z)))//.asInstanceOf[apply[s, z]]
    type apply[s <: Set, z <: Set] = s#clear#addSeq[s#toList#filter[Pred[z]]]
