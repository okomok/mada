...
object DivMod {
     def apply[x <: Dense, y <: Dense](x: x, y: y): apply[x, y] =
        `if`(y.size.lt(x.size), Then(x, y), Else(x, y)).apply.asInstanceOfProduct2.asInstanceOf[apply[x, y]]
    type apply[x <: Dense, y <: Dense] =
        `if`[y#size#lt[x#size], Then[x, y], Else[x, y]]#apply#asInstanceOfProduct2
    ...
    case class Next[x <: Dense, y <: Dense, quot <: Dense, canMinus <: Dense](x: x, y: y, quot: quot, canMinus: canMinus) extends Function0 {
        ...
        override  def apply: apply = Tuple2(quot.plus(r._1.asInstanceOfNat), r._2)
        override type apply        = Tuple2[quot#plus[r#_1#asInstanceOfNat], r#_2]
    }
