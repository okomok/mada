...
object Init {
    ...
    implicit def _ofNil[h, t <: List](implicit _prependReversed: PrependReversed[Nil, t]) = new Init[Cons[h, t], Nil] {
        override def apply(_r: Cons[h, t], _l: Nil) = _prependReversed(Nil, _r.tail)
    }

    implicit def _ofCons[r <: List, h, t <: List](implicit _init: Init[Cons[h, r], t]) = new Init[r, Cons[h, t]] {
