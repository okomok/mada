...
private[mada] final case class Match[xs <: Dense, ys <: Dense, nn <: Function0, nc <: Function0, cn <: Function0, cc <: Function0](xs: xs, ys: ys, nn: nn, nc: nc, cn: cn, cc: cc) extends Function0 {
    ...
    override  def apply: apply = xs.isEmpty.`if`(ys.isEmpty.`if`(nn, nc), ys.isEmpty.`if`(cn, cc)).apply.asInstanceOf[apply]
    override type apply = xs#isEmpty#`if`[ys#isEmpty#`if`[nn, nc], ys#isEmpty#`if`[cn, cc]]#apply
}
