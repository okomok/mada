...
private[mada] final case class SubtractNil[ys <: Nat](ys: ys) {
     def apply: apply = ys.ifNil(Always0(NatNil), Else()).apply.asInstanceOfNat
    type apply = ys.ifNil[Always0[NatNil], Else]#apply#asInstanceOfNat
...
private[mada] final case class SubtractCons[x <: Boolean, xs <: Nat, ys <: Nat](x: x, xs: xs, ys: ys) {
