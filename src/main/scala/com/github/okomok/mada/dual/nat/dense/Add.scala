...
private[mada] final class Add {
     def apply[xs <: Dense, ys <: Dense](xs: xs, ys: ys): apply[xs, ys] =
        Match(xs, ys, const0(Nil), const0(ys), const0(xs), ConsMatch(xs, ys, new TT(xs, ys), new XF(xs, ys), new FX(xs, ys), new XF(xs, ys))).apply.asInstanceOfNatDense.asInstanceOf[apply[xs, ys]]
    type apply[xs <: Dense, ys <: Dense] =
        Match[xs, ys, const0[Nil], const0[ys], const0[xs], ConsMatch[xs, ys, TT[xs, ys], XF[xs, ys], FX[xs, ys], XF[xs, ys]]]#apply#asInstanceOfNatDense
