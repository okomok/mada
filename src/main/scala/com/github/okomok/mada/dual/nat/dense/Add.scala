...
private[mada] final class Add {
     def apply[xs <: Dense, ys <: Dense](xs: xs, ys: ys): apply[xs, ys] = new Match().apply(xs, ys, always0(Nil), always0(ys), always0(xs), ConsMatch(xs, ys, new TT(xs, ys), new XF(xs, ys), new FX(xs, ys), new XF(xs, ys))).asInstanceOfNatDense.asInstanceOf[apply[xs, ys]]
    type apply[xs <: Dense, ys <: Dense] = Match#apply[xs, ys, always0[Nil], always0[ys], always0[xs], ConsMatch[xs, ys, TT[xs, ys], XF[xs, ys], FX[xs, ys], XF[xs, ys]]]#asInstanceOfNatDense
