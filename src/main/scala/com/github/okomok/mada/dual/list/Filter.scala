...
private[mada] class FilterCons {
     def apply[x <: Any, xs <: List, f <: Function1_Any_Boolean](x: x, xs: xs, f: f): apply[x, xs, f] = f.apply(x).if_List(new Then(x, xs, f), new Else(x, xs, f)).apply
    type apply[x <: Any, xs <: List, f <: Function1_Any_Boolean] = f#apply[x]#if_List[Then[x, xs, f], Else[x, xs, f]]#apply
