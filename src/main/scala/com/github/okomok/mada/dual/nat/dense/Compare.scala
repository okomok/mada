...
private[mada] final class Equals {
     def apply[xs <: Dense, ys <: Dense](xs: xs, ys: ys): apply[xs, ys] =
        Match(xs, ys, const0(`true`), const0(`false`), const0(`false`), new CC(xs, ys)).apply.asInstanceOfBoolean
    type apply[xs <: Dense, ys <: Dense] =
        Match[xs, ys, const0[`true`], const0[`false`], const0[`false`], CC[xs, ys]]#apply#asInstanceOfBoolean

    class CC[xs <: Dense, ys <: Dense](xs: xs, ys: ys) extends Function0 {
        override  val self = this
        override type self = CC[xs, ys]
        override  def apply: apply = `if`(xs.head !== ys.head, const0(`false`), new Else(xs, ys)).apply.asInstanceOf[apply]
...
private[mada] final class LessThan {
     def apply[xs <: Dense, ys <: Dense](xs: xs, ys: ys): apply[xs, ys] =
        Match(xs, ys, const0(`false`), const0(`true`), const0(`false`), ConsMatch(xs, ys, new XXTF(xs, ys), new XXTF(xs, ys), new FT(xs, ys), new XXTF(xs, ys))).apply.asInstanceOfBoolean.asInstanceOf[apply[xs, ys]]
    type apply[xs <: Dense, ys <: Dense] =
        Match[xs, ys, const0[`false`], const0[`true`], const0[`false`], ConsMatch[xs, ys, XXTF[xs, ys], XXTF[xs, ys], FT[xs, ys], XXTF[xs, ys]]]#apply#asInstanceOfBoolean
