...
private[mada] final case class Equals[xs <: Dense, ys <: Dense](xs: xs, ys: ys) {
     def apply: apply = Match(xs, ys, always0(`true`), always0(`false`), always0(`false`), CC()).apply.asInstanceOfBoolean
...
private[mada] final case class LessThan[xs <: Dense, ys <: Dense](xs: xs, ys: ys) {
     def apply: apply = Match(xs, ys, always0(`false`), always0(`true`), always0(`false`), ConsMatch(xs, ys, XXTF(), XXTF(), FT(), XXTF())).apply.asInstanceOfBoolean.asInstanceOf[apply]
