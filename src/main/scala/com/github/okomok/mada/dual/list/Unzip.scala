...
private[mada] final class Unzip {
     def apply[xs <: List](xs: xs): apply[xs] = `if`(xs.isEmpty, const0(Tuple2(Nil, Nil)), new Else(xs)).apply.asInstanceOfProduct2
    type apply[xs <: List] = `if`[xs#isEmpty, const0[Tuple2[Nil, Nil]], Else[xs]]#apply#asInstanceOfProduct2
