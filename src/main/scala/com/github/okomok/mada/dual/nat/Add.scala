...
private[mada] object Plus extends Function2[Nat, Nat, Nat] {
    override  def apply[x <: Nat, y <: Nat](x: x, y: y): apply[x, y] = x.foldRight_Nat(y, step)
    override type apply[x <: Nat, y <: Nat] = x#foldRight_Nat[y, step]
