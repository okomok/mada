...
private[mada] case class ParallelMap[Z, A](_1: Vector[Z], _2: Z => A, _3: Int) extends Forwarder[A] {
...
//    override def seek(p: A => Boolean) = _1.parallelBy(_3).seek(p compose _2).map(_2) // seek-map fusion

    // Impossible: parallel.reduce is implemented by map-reduce.
    // override def reduce(op: (A, A) => A) = _1.map(_2).parallelBy(_3).reduce(op) // reduce-map fusion
}
