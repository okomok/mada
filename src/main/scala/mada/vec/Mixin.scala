...
private[mada] class MixinVector[A](override val self: Vector[A], mx: Mixin) extends VectorProxy[A] {
  ...
  // regions
    ...
    override def dropWhile(p: A => Boolean): Vector[A] = affectMixin(mixed.dropWhile(p))
    override def takeWhile(p: A => Boolean): Vector[A] = affectMixin(mixed.takeWhile(p))
  // division
    ...
    override def span(p: A => Boolean): (Vector[A], Vector[A]) = affectMixin2(mixed.span(p))
    override def break(p: A => Boolean): (Vector[A], Vector[A]) = affectMixin2(mixed.break(p))
  // as list
    override def tail: Vector[A] = affectMixin(mixed.tail)
  // filter
    override def filter(p: A => Boolean): Vector[A] = affectMixin(mixed.filter(p))
    override def mutatingFilter(p: A => Boolean): Vector[A] = affectMixin(mixed.mutatingFilter(p))
    override def remove(p: A => Boolean): Vector[A] = affectMixin(mixed.remove(p))
    override def mutatingRemove(p: A => Boolean): Vector[A] = affectMixin(mixed.mutatingRemove(p))
    override def partition(p: A => Boolean): (Vector[A], Vector[A]) = affectMixin2(mixed.partition(p))
  // map
