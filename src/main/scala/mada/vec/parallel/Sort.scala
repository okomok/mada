...
private[mada] object SortWith {
    ...
    def apply[A](v: Vector[A], lt: Functions.Compare[A], grainSize: Int): Vector[A] = {
        Assert(!v.isParallel)
    ...
    // See: stl.IntroSort
    def loop[A](fs: ArrayList[() => Unit], grainSize: Int, * : Vector[A], __first: Int, last: Int, depth_limit: Int, __comp: Functions.Compare[A]): Unit = {
        var __last = last
