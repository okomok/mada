...
class Symbols[A](vs: Vector[A]*)(implicit c: A => Ordered[A]) extends Parser[A] {
...
class TSTreeMap[A, V](lt: (A, A) => Boolean) {
    ...
    def parse(key_ : Vector[A]): Long = {
        val (first, last) = key_.toPair
        parse(key_, first, last)
    }
    ...
    def get(key: Vector[A], first: Long, last: Long): Option[V] = {
        val (n, _, _) = Node.parse(root, key, first, last)
        if (n == null) None else n.value
    }

    def parse(key_ : Vector[A], first: Long, last: Long): Long = {
        val (_, n, i) = Node.parse(root, key_, first, last)
    ...
    object Node {
        ...
        def parse(_first1: Node, key2: Vector[A], _first2: Long, last2: Long): (Node, Node, Long) = {
            Assert(_first1 != null)
            var first1 = _first1
            var cur1: Node = null
            var first2 = _first2
