...
trait Parameterize { this: Functions.type =>
    ...
    def parameterize2[T1, T2, R](f: Function2[T1, T2, R])(q1: Parameter[T1], q2: Parameter[T2]): Function1[Seq[Parameter[_]], R] = new Function1[Seq[Parameter[_]], R] {
        override def apply(ps: Seq[Parameter[_]]) = f(getArg(ps, q1), getArg(ps, q2))
    }

    def parameterize3[T1, T2, T3, R](f: Function3[T1, T2, T3, R])(q1: Parameter[T1], q2: Parameter[T2], q3: Parameter[T3]): Function1[Seq[Parameter[_]], R] = new Function1[Seq[Parameter[_]], R] {
