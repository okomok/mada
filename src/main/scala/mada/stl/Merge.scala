...
private[mada] object Merge {
    def apply[A](v1 : Vector[A], first1: Int, __last1: Int, v2 : Vector[A], first2: Int, __last2: Int, ^ : Vector[A], result: Int, __comp: Compare.Predicate[A]): Int = {
        var __first1 = first1
        var __first2 = first2
        var __result = result
...
private[mada] object MergeSort {
    ...
    def loop[A](* : Vector[A], first: Int, __last: Int, ^ : Vector[A], result: Int, step_size: Int, __comp: Compare.Predicate[A]): Unit = {
        var __first = first
        var __result = result
