...
private[mada] object LinearInsert {
    def apply[A](* : Vector[A], __first: Int, __last: Int, __val: A, __comp: Functions.Compare[A]): Unit = {
...
private[mada] object UnguardedLinearInsert {
    def apply[A](* : Vector[A], last: Int, __val: A, __comp: Functions.Compare[A]): Unit = {
        var __last = last
        var __next = __last
