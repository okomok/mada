...
trait Forwarder[+A] extends Iterative[A] with SequenceForwarder[A] {
    ...
    override def _flatten[B](_this: Iterative[Sequence[B]]): Iterative[B] = around(delegate.asInstanceOf[Iterative[Sequence[B]]].flatten)
    ...
    override def _unsplit[B](_this: Iterative[Sequence[B]], sep: Iterative[B]): Iterative[B] = around(delegate.asInstanceOf[Iterative[Sequence[B]]].unsplit(sep))
