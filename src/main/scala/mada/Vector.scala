...
trait Vector[A] {
    ...
    def break(p: A => Boolean): (Vector[A], Vector[A]) = Break(this, p)
    def clear: Vector[A] = Clear(this)
    def contains(e: Any): Boolean = Contains(this, e)
    def count(p: A => Boolean): Long = Count(this, p)
    ...
    def elements: Iterator[A] = Elements(this)
    ...
    def reduceLeft[B >: A](op: (B, A) => B): B = ReduceLeft(this, op)
    def reduceRight[B >: A](op: (A, B) => B): B = ReduceRight(this, op)
