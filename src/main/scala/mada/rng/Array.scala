...
object ArrayCompatible extends ArrayCompatible; trait ArrayCompatible {
    implicit def madaRng_Array2ExprRng[A](from: Array[A]): ExprV2.Of[Rng[A]] = FromArrayExpr(ExprV2.Constant(from)).expr
}
...
object ArrayToRng extends ArrayToRng; trait ArrayToRng extends Predefs {
    class MadaRngArrayToRng[A](_1: ExprV2.Of[Array[A]]) {
        def toRng = FromArrayExpr(_1).expr
    }
    implicit def toMadaRngArrayToRng[A](_1: ExprV2.Of[Array[A]]): MadaRngArrayToRng[A] = new MadaRngArrayToRng[A](_1)
}

case class FromArrayExpr[A](_1: ExprV2.Of[Array[A]]) extends ExprV2[Array[A], Rng[A]] {
    override protected def _eval[U](x: ExprV2[Rng[A], U]): U = x match {
...
object ToArray extends ToArray; trait ToArray extends Predefs {
    class MadaRngToArray[A](_1: ExprV2.Of[Rng[A]]) {
        def toArray = ToArrayExpr(_1).expr
    }
    implicit def toMadaRngToArray[A](_1: ExprV2.Of[Rng[A]]): MadaRngToArray[A] = new MadaRngToArray[A](_1)
}

case class ToArrayExpr[A](override val _1: ExprV2.Of[Rng[A]]) extends ExprV2.Method[Rng[A], Array[A]] {
...
object ToArrayImpl {
    def apply[A](x: ExprV2.Of[Rng[A]]): Array[A] = x.eval.traversal match {
        case _: ForwardTraversal => inForward(x)
        case _: SinglePassTraversal => inForward(x.force)
    }

    private def inForward[A](x: ExprV2.Of[Rng[A]]): Array[A] = {
