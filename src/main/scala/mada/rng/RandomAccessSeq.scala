...
object RandomAccessSeqToRng extends RandomAccessSeqToRng; trait RandomAccessSeqToRng extends Predefs {
    class MadaRngRandomAccessSeqToRng[A](_1: Expr.Of[RandomAccessSeq[A]]) {
        def toRng = FromRandomAccessSeqExpr(_1).expr
    }
    implicit def toMadaRngRandomAccessSeqToRng[A](_1: Expr.Of[RandomAccessSeq[A]]): MadaRngRandomAccessSeqToRng[A] = new MadaRngRandomAccessSeqToRng[A](_1)
}

case class FromRandomAccessSeqExpr[A](_1: Expr.Of[RandomAccessSeq[A]]) extends Expr.Alias[RandomAccessSeq[A], Rng[A]] {
